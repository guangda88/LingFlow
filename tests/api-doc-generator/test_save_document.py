...
class TestSaveDocument:
    ...
    def test_save_yaml_without_pyyaml(self, tmp_path):
        ...
        with patch('builtins.__import__', side_effect=lambda name, *args: (_ for _ in ()).throw(ImportError(f'No module named {name}')) if name == 'yaml' else __import__(name, *args)):
            save_document(doc, str(output_path), 'yaml')
...
class TestSaveDocumentIntegration:
    ...
    def test_save_multiple_documents(self, tmp_path):
        ...
        docs = [
            {'openapi': '3.0.0', 'info': {'title': f'API {i}'}, 'paths': {}}
            for i in range(5)
        ]
