...
class RuleExtractor:
    ...
    def __init__(self, min_frequency: int = 3, min_confidence: float = 0.7, max_rules: int = 1000):
    ...
    def extract_rules(
        self, feedback_items: List[FeedbackItem], category: Optional[FeedbackCategory] = None
    ) -> List[LearnedRule]:
        """从反馈项中提取规则
        ...
        # 过滤类别
        if category:
            feedback_items = [item for item in feedback_items if item.category == category]
    ...
    def _extract_keywords(self, items: List[FeedbackItem]) -> List[str]:
        """提取关键词"""
        all_text = " ".join([f"{item.message} {item.rule_name} {item.suggestion or ''}" for item in items]).lower()
        ...
        keywords = [word for word, count in word_counts.most_common(15) if word not in stop_words and count >= 2]
    ...
    def _generate_description(self, items: List[FeedbackItem]) -> str:
        ...
        return (
            f"{base_desc} " f"(Detected by {len(tool_names)} tool{'s' if len(tool_names) > 1 else ''}, " f"{occurrence_text})"
        )
...
class RuleDeduplicator:
    ...
    def _compute_rule_hash(self, rule: LearnedRule) -> str:
        ...
        combined = f"{rule.category.value}:" f"{keywords_str}:" f"{patterns_str}"
...
class RuleValidator:
    ...
    def __init__(self, min_quality_score: float = 0.5, min_tool_support: int = 1):
