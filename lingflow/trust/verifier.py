...
class CommandOutputVerifier(Verifier):
    ...
    def verify(self, claim: TaskClaim, actual_result: Any = None) -> VerificationResult:
        try:
            ...
            return VerificationResult(
                level=self.level, passed=False, detail=f"命令输出验证失败: {e}", evidence="Exception", confidence=0.0
            )
...
class DirectoryStructureVerifier(Verifier):
    ...
    def verify(self, claim: TaskClaim, actual_result: Any = None) -> VerificationResult:
        try:
            ...
            return VerificationResult(
                level=self.level, passed=False, detail=f"目录验证失败: {e}", evidence="Exception", confidence=0.0
            )
...
class GitDiffVerifier(Verifier):
    ...
    def verify(self, claim: TaskClaim, actual_result: Any = None) -> VerificationResult:
        try:
            ...
            if not diff_output:
                return VerificationResult(
                    level=self.level, passed=False, detail="工作区干净，无更改", evidence="Empty diff", confidence=1.0
                )
            ...
            return VerificationResult(
                level=self.level, passed=False, detail="git 命令未找到", evidence="git not in PATH", confidence=0.0
            )
        except Exception as e:
            return VerificationResult(
                level=self.level, passed=False, detail=f"Git 验证失败: {e}", evidence="Exception", confidence=0.0
            )
...
class VerificationPipeline:
    ...
    def execute(self, claim: TaskClaim, actual_result: Any = None) -> VerificationResult:
        ...
        report = VerificationReport(
            claims=[claim] if claim else [], results=self.verification_results, overall_confidence=overall_confidence
        )
        ...
        return (
            ...
            else VerificationResult(level=VerificationLevel.SYNTAX, passed=False, detail="No results", confidence=0.0)
        )
    ...
    def generate_report(self) -> VerificationReport:
        ...
        report = VerificationReport(
            claims=[self.claim] if self.claim else [], results=self.verification_results, overall_confidence=overall_confidence
        )
        report.passed = overall_confidence >= 0.8
