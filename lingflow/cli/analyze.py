...
@click.option("--metrics", "-m", help="指标列表 (逗号分隔): complexity, duplication, security, maintainability")
@click.option("--output", "-o", help="输出报告路径")
@click.option("--format", "-f", type=click.Choice(["json", "markdown", "html"]), default="markdown", help="输出格式")
...
@click.option("--target", "-t", default=".", help="目标路径")
@click.option("--threshold", type=int, default=10, help="复杂度阈值")
@click.option("--verbose", "-v", is_flag=True, help="详细输出")
                ...
                if isinstance(node, ast.FunctionDef):
                    ...
                    if complexity > threshold:
                        high_complexity_functions.append(
                            {"file": str(py_file), "function": node.name, "line": node.lineno, "complexity": complexity}
                        )
...
@click.option("--target", "-t", default=".", help="目标路径")
@click.option("--min-lines", type=int, default=10, help="最小重复行数")
def analyze_duplication(target, min_lines):
                ...
                if stripped and not stripped.startswith("#"):
                    ...
                    if len(current_block) >= min_lines:
                        ...
                        code_blocks[block_key].append(
                            {"file": str(py_file), "start": block_start + 1, "lines": len(current_block)}
                        )
                    current_block = []
                    block_start = i + 1
        except Exception:
    ...
    for block_key, occurrences in code_blocks.items():
        if len(occurrences) > 1:
            duplicates.append({"block": block_key[:50] + "...", "count": len(occurrences), "occurrences": occurrences})
...
def _generate_analysis_report(report_path: Path, metrics: List[str], results: Dict, format: str):
    """生成分析报告"""
    if format == "json":
        content = json.dumps(
            {
