...
class SamplingConfig:
    ...
    # 关键指标（不采样）
    critical_metrics: set = field(default_factory=lambda: {
        "skill_load",
        "workflow_execution",
        "error_handler",
    })
...
class SamplingMonitor:
    ...
    def __init__(
        ...
        base_monitor: PerformanceMonitor = None
    ):
    ...
    def adjust_sampling_rate(self, system_load: float):
        ...
        if system_load < self._config.low_load_threshold:
            ...
            self._current_rate = self._config.low_load_rate * (1 - ratio) + \
                                 self._config.high_load_rate * ratio
