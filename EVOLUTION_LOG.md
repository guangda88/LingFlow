...
## #019 IMA批量嵌入完成 — 101,607/101,707 (99.9%)
...
| 成功嵌入 | 101,597 |
| 失败 | 100 |
| 覆盖率 | 99.9% (101,607 staged / 101,707 total) |
| 耗时 | 5,717秒 (95.3分钟) |
| 吞吐 | ~18条/秒 |
| 模型 | BGE-M3 |
...
100条失败全部来自06:07:15的一次瞬时服务器断连（embedding服务Docker容器短暂不可达）。失败记录集中在id=19135-19146范围，全部为中医(TCM)类别文档。断连恢复后job自动续传，未产生连锁失败。
...
3. **99.9%覆盖率可接受** — 100条缺失不影响检索质量（对应RAG评估中中医领域10% match rate的瓶颈不在embedding覆盖，而在query-document语义匹配）。

### 硬化措施

- staging表`ON CONFLICT (id) DO UPDATE`天然支持resume，无需外部checkpoint
- 后续批量嵌入job应增加：连接断开时重建连接池 + 失败批次记录到单独表以便精准补嵌
- 100条缺失记录可作为定向补嵌任务，优先级低
